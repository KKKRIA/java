...
<body>
	...
	<script type="text/javascript">
		...
		var name=document.getElementById("getBtn").onclick=function(){
			//입력태그의 입력값을 제공받아 저장
			...
			name=encodeURIComponent(name); //★부호화처리
			xhr.open("get", "data_two.jsp?name="+name, true);
			xhr.send(null); //GET 방식이면 무조건 null
		...
		document.getElementById("postBtn").onclick=function() {
			...
			//해결법) [application/x-www-form-urlencoded]형식으로 값이 전달되도록 리퀘스트 메시지의 헤더 정보 변경
		...
		//전달값이 많으면 GET 방식보다 POST방식으로 하는게 좋음
		//GET 으로 사용하면 부호화처리 [encodeURIComponent] 계속 해줘야되지만 POST 는 헤더정보 [setRequestHeader] 하나만 추가하면 됨
