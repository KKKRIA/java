...
<body>
	...
	<script type="text/javascript">
		...
		// => 서버 플렛폼에 날짜와 시간을 제공받아 사용하므로 모든 클라이언트에게 동일한 결과를 제공
		//문제점) 동일한 웹프로그램을 지속적으로 요청한 경우 브라우저 캐싱 기능에 의해 서버에서 
		//제공된 응답결과가 아닌 기존 응답결과 재사용 - 최초 응답결과를 사용하여 응답 처리
		//해결법-1) 요청 웹프로그램의 URL 주소에서 질의문자열(QuertString)를 계속 변경하여 요청
		//해결법-2) 요청 웹프로그램의 응답결과가 브라우저 캐싱기능으로 사용되지 않도록 무효화 처리
		function displaySlientTime() {
		...
		displaySlientTime();
		setInterval(displaySlientTime,1000)
	</script>	
