...
// => 현실에 존재하는 사물 또는 관념을 클래스라는 자료형으로 표현하여 프로그램에 동작할 수 있도록 객체로 생성하여 사용
...
//			...
//			메소드(Method) : 표현대상의 행위를 명령으로 제공하기 위한 함수 - 멤버함수
//			...
//		}

// => 메소드는 필드를 사용한 명령으로 필요한 기능을 제공되도록 작성
// => 클래스 내부에 선언된 모든메소드는 필드 접근 가능

//클래스를 작성할 때 필드와 메소드에는 접근 지정자를 사용하여 접근 허용 유무 설정 가능
...
// => 일반적으로 필드에 사용하는 접근 제한자  - 객체로 필드에 직접적인 접근을 제한하기 위해 사용
...
// => 속성 : 모델명, 엔진상태, 현재속도 - 필드
// => 행위 : 시동 온(On), 시동 오프(Off), 속도증가, 속도감소, 이동 중지 - 메소드
...
public class Car {
	...
	public void startEngine() { // 시동 온(On) , 결과 반환안할꺼라서 보이드
		engineStatus = true;
	...
	// 은닉화 처리된 필드를 위해 필드값을 반환하는 Getter 메소드와 필드값 변경하는 Setter 메소드 선언 - 캡술화
