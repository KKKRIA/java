...
//Runtime 클래스 : Java 프로그램에서 운영체제(Operation System)관련 정보을 제공하는 메소드가 선언된 클래스 
...
public class RuntimeApp {
	public static void main(String[] args) throws IOException {
		...
		System.out.print("메모리 정리하기 전 JVVM 사용 메모리의 크기 >>");
		//Runtime.totalMemory() : JVVM이 사용 가능한 전체 메모리의 크기를 반환하는 메소드
		//Runtime.freeMemory() : JVVM이 사용 가능한 여유 메모리의 크기를 반환하는 메소드
		System.out.println( (runtime1.totalMemory()-runtime1.freeMemory())+"Byte");
		
		//runtime1.gc(): 메모리를 정리하는 프로그램(Garbage Collector)를 실행하는 메소드
		runtime1.gc(); //System.gc();
		
		System.out.print("메모리 정리하기 한 후 JVVM 사용 메모리의 크기 >>");
		System.out.println( (runtime1.totalMemory()-runtime1.freeMemory())+"Byte");
