...
// => 메소드에 작성된 명령을 실행하기 위해 필요한 값을 메소드 호출시 전달된 값을 저장하기 위한 변수
...
// =>매개변수에 값이 정상적으로 전달되지 않으면 메소드 호출 실패

//return 키워드 : 메소드를 강제로 종료하기 위한 키워드
//형식-1) if(조건식) return;
//=> 메소드의 반환형을[void]로 작성
//형식-2) return 반환값;
// => 메소드를 종료하면서 반환값(변수 뚜는 연산식)을 메소드를 호출하는 명령에게 제공
...
public class Method { //클래스 안에서는 입출력안함
	
	void displayOne() {
		System.out.println("Method 클래스의 displatOne() 메소드 호출");
	}
	void displayTwo() {
	...
	int [] returnArray() {
		...
		return array; //배열의 메모리 주소 반환 - 배열을 반환
		*/
	...
	// 매개변수 생략기호 (...)를 이용하여 매개변수를 작성하면 값을 0개이상 전달받아 
	// 메소드에서 배열처럼 사용가능
