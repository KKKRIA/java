...
public class AcademyApp {
	public static void main(String[] args) {
		...
		//직원정보(AcademyStaff객체)를 저장하기 위한 요소들이 존재하는 배열 생성
		AcademyStaff[] staffs = new AcademyStaff[100];
		*/
		
		
		//사람정보(AcademyPerson객체)를 저장하기 위한 요소들이 존재하는 배열 생성
		// => 배열요소에 학생정보(AcademyStudent), 강사정보(AcademyInstructor), 직원정보(AcademyStaff) 저장 가능
		...
		// => 배열 요소에는 부모클래스의 객체가 저장되어 부모클래스 메소드 호출
		// => 객체형변환을 이용하면 배열 요소를 자식클래스의 객체를 일시적으로 저장하여 자식클래스의 메소드 호출 가능 
		...
		for(AcademyPerson person:persons) {
			...
			//명시적 객체 형변환을 이용하여 참조변수에 자식클래스의 객체를 일시적으로 저장하여 자식클래스의 메소드 호출 
			
			//문제점) 상속관계가 아닌 클래스로 명시적 객체 형변환 할 경우 ClassCastException이 발생
			//해결볍) 참조변수로 객체형변환 가능한 클래스를 확인한 후 명시적 객체 형변환 이용
			//=> instance of 연산자를 사용하여 참조변수의 객체 형변환 가능 클래스 검사
			
			// 형식) 참조변수 instance of 클래스
			//=> 참조변수로 참조 가능한 클래스인 경우 [false] 또는 [true]를 제공하는 연산자
			...
			//오버라이드 선언된 자식클래스의 메소드는 묵시적 객체형변환에 의해 부모클래스의 메소드를 호출하지 않고 자식클래스의 메소드 호출
