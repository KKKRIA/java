...
// => JDK11 이상에서는 정적메소드(Static Method)와 기본메소드 (Default Method)도 선언가능

//형식) [public] interface 인터페이스명{
//			자료형 필드명 =값; //public static final 생략가능  (상수필드)
//  		...
//			반환형 메소드명(자로형 매개변수, ...); //public abstract 생략가능
//			...
//		}

// => 인터페이스의 이름은 파스칼표기법(첫문자는 대문자-클래스처럼)을 사용하여 작성하는 것을 권장

//인터페이스는 클래스가 상속받아 사용하기 위한 자료형 - 다중 상속가능
//형식) public class (자식)클래스명 implements (부모)인터페이스, 인터페이스명, ...{}

// => 인터페이스를 상송받은 자식클래스는 무조건 상속받은 부모 인터페이스의 모든 추상메소드를 오버라이드 선언
...
//ex) public class 늑대인간 extends 인간, 늑대   => 불가능 (여러개 다중상속 안됨)
//	   public class 늑대인간 extends 인간 implements 늑대 (클래스아님,인터페이스)   => 가능
//	   public class 흡혈늑대인간 extends 인간 implements 늑대, 흡혈귀   => 가능

//2. 클래스에 대한 작업지시서의 역할을 제공하기 위해 인터페이스 사용
//ex) TV or RADIO or SMARTPHONE >> 볼륨증가, 볼륨감소 -인터페이스
// => 인터페이스를 상속받은 모든 자식클래스에 동일한 형태의 메소드가 선언되도록 규칙 제공 (자식이 바뀌어도 ok )
...
public class WolfHumanApp {
	public static void main(String[] args) {
		...
		//인터페이스(부모)로 참조변수를 생성하여 자식클래스의 객체 저장
