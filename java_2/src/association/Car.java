...
// => 인터페이스 : 현식에 존재하는 대상을 클래스 보다 추상적으로 표현하기 위한 자료형

//3.연관관계(Association) : 포함 관계 - X has a Y  
// => 직접 연관 관계(Direct Association) : 한 방향으로만 도구로써 기능을 제공하는 관계
// => 컴퓨터 << CPU + MainBoard + Memory

//4.집합 연관관계 (Aggregation) : 포함관계로 설정된 객체들의 생명주기가 다른 포함 관계
// => 컴퓨터 << 프린터

//5.복합 연관관계 (Composition) : 포함관계로 설정된 객체들의 생명주기가 같은 포함 관계
//=> 게임<< 캐릭터

//6.의존관계 (Dependency) : 포함관계로 설정된 객체를 변경되어도 다른 객체에 영향을 주지 않는 포함관계
//=> TV << 리모콘



//자동차의 정보(모델명, 생산년도, 엔진정보)를 저장하기 위한 클래스
public class Car {
	private String modelName;
	private int productionYear;
	//엔진정보를 저장하기 위한 필드 - Engine 클래스 자료형으로 선언된 필드
	...
	public void displayCar(){
		...
		//=>포함관계가 설정되지 않은 상태에서 메소드가 호출될 경우 NullPoint
