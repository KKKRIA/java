...
// => Java 자료형을 보다 쉽게 관리하기위해 패키지를 사용
// => 패키지의 이름은 도메인을 역방향으로 나열하고 그룹명을 지정하여 작성하는 것을 권장

//ex) itwill.xyz : 도메일 >> xyz.itwill.board
//도메인(Domain) : 인터넷에서 개인 또는 그룹이 사용하기 위한 네트워크 식별자


//패키지에 작성된 소스파일의 처음 위치에는 소스파일이 작성된 패키지를 반드시 명확하게 작성
//형식) package 패지키경로; 
// => Java자료형이 어떤 패키지에 선언된 자료형인지를 명확하게 구분하여 사용



//import 키워드 : 다른 패키지에 작성된 Java 자료형을 명확히 표현하여 접근하기 위한 키워드
//형식) import 패키지경로.자료형;   >> 자료형 대신 * (전체)사용가능
// => package 키워드로 작성된 명령 하단에 선언하여 자료형 위에 선언
// => 이클립스에서는 다른 패키지에 작성된 자료형을 사용할 경우 import 명령을 자동 완성하는 기능 제공 - [Ctrl] + [Space]=> 같은이름의 자료형이 여러개 있으면 선택잘하기
// => [Ctrl] + [Shift] + [O]  >> import 명령을 정리하는 단축키 : 
//    불필요한 자료형에 대한 import 설정을 제거하거나 필요한 자료형에 대한 import설정을 추가하는 기능 제공 
...
public class SubjectApp {
	public static void main(String[] args) {
		...
		//다른패키지에 작성된 클래스를 import처리한 경우 패키지 경로 표현없이 클래스 접근 가능
