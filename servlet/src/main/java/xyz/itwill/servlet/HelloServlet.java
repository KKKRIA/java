...
// => 웹프로그램은 클라이언트 요청으로 WAS(Web Application Server)에 의해 실행되며 실행결과로
//웹문서를 동적으로 생성하여 클라이언트에게 응답
...
// => HttpServlet클래스를 상속받은 자식클래스는 객체 직렬화 클래스로 serialVersionUID 필드를 선언하는 것을 권장

public class HelloServlet extends HttpServlet{
	private static final long serialVersionUID = 1L;
	
	//2.doGet() 메소드 또는 doPost() 메소드를 오버라이드 선언
	// => 클라이언트 요청에 대한 처리 명령과 실행결과를 웹문서로 생성하는 명령을 작성하는 
	...
	//WAS 프로그램에 의해 요청처리 메소드가 호출될 때 메소드 매개변수에는 HttpServletRequest
	//객체(request 객체)와 HttpServletResponse response 객체(response 객체)가 전달되어 저장 
	
	@Override
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//★HttpServletRequest request 객체 : 리퀘스트 메세지(★요청 관련 정보)를 표현한 객체 //(줄여서 request 객체라 부름)
		//★HttpServletResponse response 객체 : 리스폰즈 메세지(★응답 관련 정보)를 표현한 객체 //(줄여서 response 객체라 부름)
		
		//3.클라이언트 요청에 대한 실행결과를 [응답]하기 위한 파일형태(MimeType)를 변경
		...
		//기본 응답파일 형식: text/html;charset=iso-8859-1 >> 서유럽어로 만드어진 HTML 문서로 응답
		response.setContentType("text/html;charset=utf-8"); // 한글로 만드어진 HTML 문서로 응답
		
		//4.응답 파일을 생성하기 위한 출력스트림을 반환받아 저장
		//HttpServletResponse.getOutputStream() : 응답파일을 생성하기 위한 출력스트림(ServletOutputStream)을 반환하는 메소드
		// => 원시데이타(1Byte)를 전달해 그림, 소리, 동영상 파일등과 같은 이진파일 생성하기 위한 출력스트림
		//HttpServletResponse.getWriter() : 응답파일을 생성하기 위한 출력스트림(PrintWriter 객체)을 반환하는 메소드
		// => 문자데이타(2Byte)를 전달해 HTML, XML 파일등과 같은 문서파일 생성하기 위한 출력스트림
		...
		//객체로 생성하여 요청처리 메소드를 호출하고 처리결과를 파일로 생성하여 클라이언트에게 전달하여 응답
		// => 클라이언트가 요청한 서블릿에 대한 클래스가 이미 객체로 생성된 경우 요청 처리 메소드만
		//호출하여 실행
	}
