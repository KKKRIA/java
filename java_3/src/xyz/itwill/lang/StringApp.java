...
public class StringApp {
	public static void main(String[] args) {
		...
		//문자열은 " "기호를 사용하여 표현하면 자동으로 메모리의 메소드영역(MethodArea) String 객체를 생성하여 String 객체에 문자열 저장
		// => 메모리의 메소드영역에는 동일한 문자열이 저장된 String 객체를 생성 불가능
		String str1="ABC"; //String 객체를 생성하여 참조변수에 메모리주소 저장  //값이 아니고 객체(ABC) //메소드영역
		...
		// ""기호로 String 객체를 표현할 경우 동일한 문자열의 String 객체를 생성하지 않고 기존에 생성된 String 객체를 재활용하여 사용
		...
		if(str1==str2) {
			System.out.println("str1 변수와 str2변수의 저장된 String 객체의 메모리 주소가 같습니다");
		}else {
			System.out.println("str1 변수와 str2변수의 저장된 String 객체의 메모리 주소가 다릅니다");
		}
		System.out.println("====================================================");
		
		
		
		//new 연산자로 생성자를 호출하여 String객체를 생성하면 메모리의 힙영역(HeapArea)에 새로운 String객체를 생성
		String str3=new String("ABC"); //힙영역에 새로 만든거
		
		if(str1==str3) {
			System.out.println("str1 변수와 str3변수의 저장된 String 객체의 메모리 주소가 같습니다");
		}else {
			System.out.println("str1 변수와 str3변수의 저장된 String 객체의 메모리 주소가 다릅니다");
		}
		System.out.println("====================================================");
		
		
		//★String.equals(String str) : String 객체에 문자열과 매개변수로 전달받은 문자열을 비교하여 다른경우 [false]를 반환하고 같은 경우[true]를 반환하는 메소드
		// => 비교문자열이 영문자인 경우 대소문자를 구분하여 비교
		if(str1.equals(str3)) {
			System.out.println("str1변수와 str3 변수에 저장된 String 객체의 문자열이 같습니다.");
		} else {
			System.out.println("str1변수와 str3 변수에 저장된 String 객체의 문자열이 다릅니다.");
		}
		System.out.println("====================================================");
		...
		//★String.equalsIgnoreCase(String str) : String 객체에 문자열과 매개변수로 전달받은 문자열을 비교하여 다른경우 [false]를 반환하고 같은 경우[true]를 반환하는 메소드
		// => 비교문자열이 영문자인 경우 대소문자를 구분하지 않고 비교
		...
		//String 객체의 문자열이 큰 경우 양수를 반환하고 매개변수로 전달받은 문자열이 큰 경우 음수를 반환하면 같은 경우 0을 반환하는 메소드
		...
		//★String.getBytes() : String 객체에 저장된 문자열을 byte배열로 변환하여 반환하는 메소드
		// => byte 배열의 요소에는 문자열을 구성하는 문자들이 코드값(정수값)으로 차례대로 저장 
		byte[] array=str1.getBytes();
		
		for(byte ch:array) {
			//System.out.print(ch); //byte배열의 요소값(정수값)을 형변환하여 출력
			System.out.print((char)ch); //byte배열의 요소값(정수값)을 형변환하여 출력
		}
		System.out.println();
		System.out.println("====================================================");
		...
		//★String.indexOf(String index) : String 객체에 저장된 문자열에서 매개변수로 전달받은 문자열(문자)를 
		//처음부터 차례대로 검색하여 시작위치값(첨자)를 반환하는 메소드
		// => 매개변수로 전달받은 문자열을 찾을 수 없는 경우 -1반환
		System.out.println("A문자열의 저장 위치 ="+str5.indexOf("A"));
		System.out.println("EFG문자열의 저장 위치 ="+str5.indexOf("EFG"));
