...
public class PanelApp extends Frame{
	...
	public PanelApp(String title) {
		...
		//Panel 클래스 : 컴퍼넌트를 배치하여 그룹화하기위한 종속적인 컨테이너 클래스
		// => 독립적으로 사용 불가능한 컨테이너이므로 다른 컨테이너에 배치하여 사용(종속적)
		Panel panel =new Panel(); //[FlowLayout]이 기본 배치관리자
		
		//패널 컨테이너에 버튼을 컴퍼넌트를 차례대로 배치
		panel.add(red);
		panel.add(green);
		panel.add(blue);
		...
		// => name: 글자의 글꼴, style: 글자 스타일, size: 글자크기
		...
		// => 빨간색, 초록색, 파란색는 0~255 범위의 정수값으로 표현
		// => Color클래스는 대표색을 상수(Color 객체)로 제공 
		...
		//Component.setEnabled(boolean b): 컴퍼넌트 활성화 상태 여부를 변경하는 메소드
		red.setEnabled(false); //컴퍼넌트 비활성화
		
		
		//TextArea.setFocusable(boolean b): 텍스트 컴퍼넌트의 입력촛점 위치 여부를 변경하는 메소드
