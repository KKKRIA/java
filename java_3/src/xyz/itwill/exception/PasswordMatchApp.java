...
public class PasswordMatchApp {
	public static void main(String[] args) {
		...
		if(number==Password) {
			System.out.println("[결과]입력된 비밀번호가 같습니다");
		}else{
			System.out.println("[결과]입력된 비밀번호가 맞지않습니다");
		}
		*/
		...
		try {
			...
			if(number!=Password) {
				...
				//throw new Exception();
				//PasswordMismatchException 클래스는 개발자가 직접 생성한 예외클래스로 예외발생시 예외처리를 하지 않을 경우 에러 발생 - 일반예외
				// =>예외가 발생되면 catch 구문으로 프로그램의 흐름이 이동
				throw new PasswordMismatchException("[결과]입력된 비밀번호가 틀립니다");
			}
			
			//예외가 발생되지 않을 경우 실행될 명령
			System.out.println("[결과]입력된 비밀번호가 같습니다");
