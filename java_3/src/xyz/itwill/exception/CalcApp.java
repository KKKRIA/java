...
public class CalcApp {
	public CalcApp() throws InputMismatchException, ArithmeticException { //생성자에서만 명령작성
	...
	public static void main(String[] args) {
		/*
		try {
		//new 연산자로 클래스의 생성자를 호출하여 객체 생성 - 생성자에 작성된 명령 실행 //메인에서는 객체만 생성
		// => 생성자를 호출하면 예외 전달되므로 예외처리 하는 것을 권장
			new CalcApp();
		...
		*/
		...
		//catch 구문에서 예외 클래스를 |연산자로 연결하여 다수의 예외 객체를 전달받아 예외처리 가능
		try {
			//new 연산자로 클래스의 생성자를 호출하여 객체 생성 - 생성자에 작성된 명령 실행 //메인에서는 객체만 생성
			// => 생성자를 호출하면 예외 전달되므로 예외처리 하는 것을 권장
				new CalcApp();
