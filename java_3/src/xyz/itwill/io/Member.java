...
//객체직렬화 (Object Serialization)  : 객체를 byte 배열(원시데이타)로 변환하여입력 또는 출력처리되기 위한 기능을 제공 

//클래스를 객체 직렬화 처리하는 방법 - 객체 직렬화 클래스
//1. Serializable 인터페이스를 상속받아 클래스 작성
// => Serializable 인터페이스의 추상메소드가 없으므로 오버라이드 선언 불필요
// => 객체의 모든 필드값을 byte배열로 변환하여 입력 또는 출력처리
...
// => 오버라이드 선언된 메소드에서 원하는 필드값만을 byte배열로 변환하여 입력 또는 출력처리
...
public class Member implements Serializable {
	...
	public Member(String id, String name, String numbe) {
		super();
		this.id = id;
		this.name = name;
		this.phone = numbe;
