...
// => 시냇물이 흐르는 모양을 모델링하여 만들어 값이 할쪽 방향으로 전달되며 순차적으로 처리

//원시데이타 기반의 스트림(Byte Stream):가공되지 않은 원시데이타를 전달하기 위한 입출력 스트림
...
public class ByteStreamApp {
	public static void main(String[] args) throws IOException{
		System.out.println("[메세지]키보드로 눌러 값을 입력해 주세요. [프로그램 종료 : Ctrl+Z]");
		...
		while(true) {
			...
			// => InputStream클래스를 기반으로 제공되는 입력스트림(객체)
			// => 키보드를 누르면 키보드의 문자값이 입력스트림으로 전달
			//InputStream.read(): 입력스트림에 존재하는 값을 원시데이타(1Byte)로 읽어 정수값(int)으로 반환하는 메소드
			// => 입력스트림에 값이 없는 경우 스레드는 일시 중지상태로 전환
			// => 입력스트림에 엔터(Enter)가 입력될 경우 스레드 다시 실행
			// => 입력스트림 또는 출력스트림 관련 메소드는 IOException발생
			// => IOException은 일반 예외이므로 반드시 예외처리
			readByte=System.in.read();
			
			//입력 종료 신호(Ctrl+Z : EOF)가 전달된 경우 반복문 종료
			if(readByte==-1)break;
			
			
			//System.out : Java 프로그램 개발을 위해 기본적으로 제공되는 모니터 출력스트림이 저장된 필드
			// => OutputStream클래스를 기반으로 제공되는 입력스트림(객체) - PrintStream클래스
			// => 출력스크림에 값을 전달하여 모니터가 출력처리
