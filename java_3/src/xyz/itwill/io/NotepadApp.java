...
public class NotepadApp extends JFrame{
	...
	public NotepadApp(String title) {
		...
		jTextArea=new JTextArea(); //텍스트작성 공간
		jTextArea.setFont(new Font("굴림체",Font.PLAIN,20));
		JScrollPane jScrollPane=new JScrollPane(jTextArea); //스크롤 생성
		
		getContentPane().add(jScrollPane, BorderLayout.CENTER);
		
		openDialog=new FileDialog(this,"열기",FileDialog.LOAD); //읽어들이기 용도의 다이얼로그
		saveDialog=new FileDialog(this,"저장",FileDialog.LOAD); //저장 용도의 다이얼로그
