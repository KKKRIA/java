...
public class FileOutputSteamApp {
	public static void main(String[] args) throws IOException {
		...
		// => 예외처리를 하지 않고 예외를 전달할 경우 매개변수로 전달받은 파일경로의 파일을 JVM이 자동으로 생성하여 출력스트림 제공
		...
		//FileOutputStream.close() : 파일출력스트림을 제거하는 메소드
		// => 파일에는 입력스트림과 출력스트림을 하나씩만 생성가능 - 다른어플리케이션에서 사용할 수 없기때문에 close() 꼭 닫아주기!!
		out.close();
