...
public class TimeClientApp {
	public static void main(String[] args) throws ClassNotFoundException {
		try {
			//Socket클래스 : TCP 프로그램에서 다른 컴퓨터와 타겟 컴퓨터와 연결을 위한 정보를 저장하기 위한 클래스
			//다른 컴퓨터의 소켓과 연력되어 값을 송수신할 수 있는 입출력 스트림 제공
			// => Socket 클래스의 Socket(String host, int port)생성자를 이용하여 매개변수에 
			//접속 컴퓨터(서버)의 네트워크 식별자 (호스트이름 또는 IP주소)와 활성화된 포트번호를 전달하여 Socket객체 생성 - 서버에 접속
			// => UnknownHostException 및 IOException 발생 - 일반 예외이므로 반드시 예외 처리
			Socket socket=new Socket("192.168.13.19",2000);
			
			//System.out.println("socket="+socket);
			
			//Socket.getInputStream() : Socket 객체의 입력스트림을 반환하는 메소드
