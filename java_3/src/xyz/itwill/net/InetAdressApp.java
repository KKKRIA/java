...
// => 프로토콜 구조 : 네트워크 계층 > 인터넷 계층(IP) > 전송계층(TCP or UDP) > 응용계층(FTP,HTTP.)
...
//TCP(Transmission Control Protocol) : 연결형 프로토콜(네트워크 기능을 이용한 연결 후 통신가능)
...
// => IPV4 : 32Bit를 이용하여 IP 주소 표현 - 10진수 0~255 범위의 정수값 4개를[.]으로 구분하여 
// => IPV6 : 128Bit를 이용하여 IP 주소 표현 - 16진수 0000~FFFF 범위의 정수값 8개를[:]으로 구분하여 
// => 공인 IP 주소와 사설 IP 주소 (A Class : 10.0.0.0 ~ 10.255.255.255, B Class : 172.16.0.0 ~ 172.31.255.255, C Class : 192.168.0.0 ~ 292.168.255.255)로 구분
...
//Gateway 주소 : 라우터에 부여된 IP주소
//DNS 서버 : 도메인(호스트)을 IP 주소로 변환하는 기능의 서비스를 제공하는 컴퓨터

//도메인(Domain) : 인터넷을 사용하기 위해 네트워크 그룹 또는 호스트에서 부여하는 문자로 된 네트워크 식별자
...
// => 0 ~ 1023 : well-Known Port(HTTP_80, HTTPS : 443, SMTP : 25, POP3 : 110 등) -> 사용불가
// => 1024 ~ 49151 : 네트워크 프로그램 작성시 사용 가능한 포트 -> 사용가능
// => 49152 ~ 65535 : 일시적으로 사용하는  임시 포트 - Private Port -> 사용불가
