...
//Date 클래스 : 날짜와 시간을 저장하여 객체에 저장된 날짜와 시간 관련 기능을 메소드로 제공하기 위한 클래스

public class DateApp {
	@SuppressWarnings("deprecation")
	public static void main(String[] args) {
		//Date 클래스의 기본 생성자를 사용하여 객체를 생성하면 시스템의 현재 날짜와 시간이 저장된 Date 객체 생성
		Date now=new Date(); //now라는 변수를 만들어 데이트 객체에 저장
		...
		// => 1900년 부터 1씩 증가된 정수값을 반환하기 때문에 +1900년 더해야함
		...
		Date wantDate=new Date(93, 7, 30); //1900년부터 시작되니까 100을 더하면 2000/ 1월 
		System.out.println("wantDate="+wantDate);
		
		long wantTime=wantDate.getTime();
		System.out.println("연산결과(일)"+(currentTime-wantTime)/(1000*86400)); //86400초
