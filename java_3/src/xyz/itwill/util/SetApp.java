...
//콜렉션 클래스(Collection Class) : 자료구조(Data Structure)를 사용하여 다수의 객체를 효율적으로 관리(추가, 삭제, 검색)하기 위한 기능을 제공하는 데이타 수집 클래스를 표현
//=> Collection 인터페이스(Set 인터페이스, List 인터페이스), Map 인터페이스 중 하나를 상속받아 작성된 클래스


//Set 인터페이스를 상속받은 콜렉션 클래스 - HachSet, TreeSet 등
// => 객체의 저장 순서가 없으며 메모리 주소와 객체에 저장된 값이 동일한 객체의 중복 저장 불가능
...
public class SetApp {
	public static void main(String[] args) {
		...
		//Set 객체에는 동일한 객체가 요소로 중복 저장되지 않는 기능 제공
		set.add("홍길동"); 
		System.out.println("set = "+ set);
		System.out.println("================================================================");
		...
		//Iterator.hasNext() : Iterator 객체로 처리가능한 요소의 존재 유무를 반환하는 메소드
		// => false : 처리요소 미존재, true : 처리요소 존재
		while(iterator.hasNext()) {
			//Iterator.next() : 커서 위치의 요소(객체)를 반환하는 메소드
			// => 커서 위치에 요소를 반환하고 커서는 다음 요소의 위치로 자동 이동
		...
		// => 향상된 for 구문을 사용하면 내부적으로 Iterator객체를 사용하여 반복  처리
		for(String str : set) {
			System.out.print(str +" ");
		}
