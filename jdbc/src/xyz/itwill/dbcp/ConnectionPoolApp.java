...
public class ConnectionPoolApp {
	public static void main(String[] args) throws SQLException {
		//ConnectionPool 클래스는 싱글톤 클래스이므로 new 연산자를 사용하여 객체를 생서아지 않고 객체를 반환받아 사용
		// => ConnectionPool 객체에는 Connection 객체가 2개 생성되어 저장
		ConnectionPool cp=ConnectionPool.getInstance();
		
		//ConnectionPool.getConnection() : ConnectionPool 객체에 저장된 Connection 객체 중 하나를 반환하는 메소드 - 다른 사용자가 사용 불가능하도록 설정
		Connection con1 = cp.getConnection();
		System.out.println("con1="+con1);
		//ConnectionPool.freeConnectionPool(Connection con) : Connection객체를 ConnectionPool 객체에 반환하는 메소드 - 다른 사용자가 사용 가능하므로 설정
