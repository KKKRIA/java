...
// => 입력과 출력은 프로그램에서 구현하고 데이타 처리는 DAO클래스의 메소드를 호출하여 처리
	...
	public void addStudent() {
		...
		try {
			...
			while(true) { // 학번 입력값을 검증하기 위한 반복문
				...
				//입력된 학번이 STUDENT 테이블에 저장된 기존 학생정보의 학번과 비교하여 중복된 경우
				//비정상적인 값이므로 재입력 처리 
				
				//학번을 전달받아 STUDENT 테이블에 저장된 해당 학번의 학생정보를 검색하여 
				//반환하는 DAO 클래스의 메소드 호출
				// => null반환 : 학생정보 미검색, StudentDTO 객체 반환 : 학생정보 검색
				//싱글톤 클래스는 객체를 반환받아 직접 메소드 호출 - 참조변수 호출
				StudentDTO stdent= StudentDAOImpl.getDAO().selectStudent(no);
				
				if(stdent != null) { //키보드로 입력된 학번의 학생정보가 검색된 경우 - 학번 중복
			...
			while(true) {
				...
				//이름에 대한 입력패턴의 정규표현식 작성하여 변수에 저장
				String nameReg="^[가-힣]{2,5}$";
				if(!Pattern.matches(nameReg, name)) {
					System.out.println("[입력오류] 이름은 2~5범위의 한글로만 입력해주세요.");
					continue;
				}
				break;
			}
			...
			while(true) {
				...
				//전화번호에 대한 입력패턴의 정규표현식 작성하여 변수에 저장
				String phoneReg="(01[016789])-\\d{3,4}-\\d{4}"; //01[다음에 들어갈 숫자]- 숫자3자리나 4자리 - 숫자4자리
				if(!Pattern.matches(phoneReg, phone)) {
					System.out.println("[입력오류] 전화번호 형식에 맞게 입력해주세요.");
					continue;
				}
				break;
			}
			...
			//키보드로 입력받은 학생정보를 STUDENT 테이블에 행으로 삽입- DAO 클래스의 메소드
			// => DTO클래스의 메소드 호출
			...
			//학생정보를 전달받아 STUDENT 테이블에 삽입하는 DAO클래스의 메소드 호출
