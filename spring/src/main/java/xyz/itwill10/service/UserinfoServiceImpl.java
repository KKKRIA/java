...
// => 매개변수에 암호화 처리할 문자열과 첨가물을 전달받아 암호화 처리 - 첨가물에 의해 비밀번호가 다르게 암호화 처리
...
public class UserinfoServiceImpl implements UserinfoSercice{
	private final UserinfoDAO userinfoDAO;
	
	@Transactional(rollbackFor = Exception.class) //인서트 딜리트 업데이트 사용시 @Transactional 사용 권장
	@Override
	public void addUserinfo(Userinfo userinfo) throws ExistuUserinfoException {
		//매개변수로 전달받은 회원정보의 아이디가 기존 회원정보의 아이디와 중복될 경우
		if(userinfoDAO.selectUserinfo(userinfo.getUserid()) != null){
			//예외를 명확하게 구분하고 예외처리시 필요한 값을 제공하기 위해 직접 생성한 예외 
			//클래스를 사용하여 인위적인 예외 발생
			throw new ExistuUserinfoException("이미 사용중인 아이디를 입력 하였습니다.",userinfo);
		}
	...
	@Override
	public void removeUserinfo(String userinfo) throws UserinfoNotFoundException {
		// TODO Auto-generated method stub
		
	}

	@Override
	public Userinfo getUserinfo(String userinfo) throws UserinfoNotFoundException {
		// TODO Auto-generated method stub
		return null;
	}
