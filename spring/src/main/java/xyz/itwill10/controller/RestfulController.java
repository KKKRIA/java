...
public class RestfulController {
	//회원정보를 입력받기 위한 JSP 문서의 뷰이름을 반환하느 요청 처리 메소드
	...
	//반환하는 요청 처리 메소드
	// => 하나의 전달값을 하나의 매개변수로 제공받아 요청 처리 메소드에서 사용 - @RequestParam 어노테이션
	@RequestMapping(value = "/join", method = RequestMethod.POST)
	public String join(@RequestParam String id, @RequestParam String name, Model model) { //String 대신 Map으로 사용하면 한꺼번에 가져올 수 있음
	...
	*/
	...
	//클라이언트에게 직접 텍스트 데이타로 응답되도록 처리하는 어노테이션
	// => 반환값을 ViewResolver 객체를 사용하여 뷰로 변환해 응답하지 않고 요청 처리 
	//메소드가 직접 응답 처리
	// => @ResponseBody 어노테이션 대신 ResponseEntity 클래스를 사용하여 응답 처리 가능
	//@ResponseBody : 리퀘스트 메시지 몸체부에 저장된 모든 전달값을 문자열로 제공받기 위한 어노테이션 
	// => POST, PUT, PATCH, DELETE 등의 요청방식에 의해 리퀘스트 메세지 몸체부에 
	//저장된 모든 전달값을 [이름 =값&이름=값&...] 형식의 문자열로 제공받아 사용 가능
	// => GET 방식으로 페이지를 요청한 경우 리퀘스트 메세지 몸치부가 비어 있으므로 @ResponseBody
	// => 페이지 요청시 JSON 형식의 텍스트 데이타로 전달된 값을 매개변수로 제공받아 저장하기 위한 텍스트 데이타를 매개변수에 전달받아 저장하기 위해 사용
	// => @ResponseBody 어노테이션 대신 ResponseEntity 클래스를 사용하여 요청 처리 가능
	@RequestMapping(value = "/join", method = RequestMethod.POST)
	@ResponseBody
	...
	//문제점) 리스폰즈 메세지 몸체부에는 Java 객체를 저장하여 응답 처리 불가능 - 406에러코드 발생
	//해결법) 요청 처리 메소드에 의해 반환되는 Java 객체를 문자열(XML 또는 JSON)로 변환하여 
	//리스폰즈 메세지 몸체부에 저장하여 응답 처리
	// => jackson-databind 라이브러리를 프로젝트에 빌드 처리하면 요청 처리 메소드에 의해 반환되어
	//Java 객체를 JSON 형식의 문자열로 자동 변환하여 응답 처리 - 메이븐 - pom.xml
 	@RequestMapping("/member")
	@ResponseBody
	public RestMember restMember() {
		//RestMember 객체를 생성하여 반환 
 		// => jackson-databind 라이브러에 의해 JSON 형식의 문자열로 자동 변환
 		// => Java 객체를 자바스크립트의 Object 객체의 형식의 JSON으로 텍스트 데이터로 변환되어   
